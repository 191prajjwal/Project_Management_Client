...
const Public = ({ taskId }) => {
  ...
  const showPublicTaskData = async (taskId) => {
    try {
      const response = await axios.get(
        `${baseUrl}/api/v1/tasks/publicview/${taskId}`
      );
  ...
  const funTotalChecks = () => {
    publicTaskData.checklist &&
      publicTaskData.checklist.map((taskList, key) => totalChecks++);
    return totalChecks;
  };
      ...
      {publicTaskData ? (
        <div className={StylePublic.public}>
          <div className={StylePublic.logo}>
            <img src={logoimg} alt="logo" style={{ width: "51px" }} />
            &nbsp;&nbsp;&nbsp;Pro Manage
          </div>
          <br />
          <div className={StylePublic.cards}>
            <div className={StylePublic.priorityText}>
              ...
              &nbsp;&nbsp;{publicTaskData.priority}
            </div>
            <br />
            <div className={StylePublic.cardTitle}>{publicTaskData.title}</div>
            <br />
            <br />
