...
const Card = ({
  ...
  const toggleBoard = async (newBoard) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${baseUrl}/api/v1/tasks/update/board`,
        { taskId, newBoard },
        {
          headers: {
  ...
  const handleChange = (changeBoard) => {
    if (changeBoard === "backlog") {
      return (
        <>
          <div
            className={StylesCard.butFooter}
            onClick={() => toggleBoard("inProgress")}
            value="inProgress"
          >
            PROGRESS
          </div>
  ...
  const funTotalChecks = (checklist) => {
    checklist.map((taskList, key) => totalChecks++);
    return totalChecks;
  };
  ...
  const deleteTask = async (taskId) => {
    ...
    try {
      const response = await axios.delete(
        `${baseUrl}/api/v1/tasks/delete/${taskId}`,
        {
          headers: {
            Authorization: `${localStorage.getItem("token")}`,
          },
  ...
  const generateShareableLink = async (taskId) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/api/v1/link/sharelink/${taskId}`
      );
  ...
  return (
    <>
      ...
      <div className={StylesCard.card}>
        <div
          className={StylesCard.priorityText}
          style={{ display: "flex", justifyContent: "space-between" }}
        >
          <div>
            <img src={imgSrc} alt="high" />
            &nbsp;&nbsp;{priority}
            &nbsp; &nbsp;
            {assignedUserName != userName && assignedUserName !== "" && (
              <span
                title={`This task is assigned to ${assignedUserName}`}
                className={StylesCard.assignedUser}
              >
          ...
          >
            ...
            {showOverlay && (
              ...
              >
                <div
                  className={StylesCard.dropDownBut}
                  onClick={() => editTask(myTaskId)}
                >
                  Edit
                </div>
                <div
                  className={StylesCard.dropDownBut}
                  onClick={() => generateShareableLink(myTaskId)}
                >
                  Share
                </div>
                <div
                  className={StylesCard.dropDownButDel}
                  onClick={() => openModal()}
                >
                  Delete
                </div>
        ...
        <div className={StylesCard.cardTitle}>
          {title.length < 45 ? title : title.slice(0, 44) + "..."}
          {title.length >= 45 && (
            <span className={StylesCard.tooltiptext}>{title}</span>
          )}
        </div>
        <div className={StylesCard.checklist}>
          Checklist ({checksMarked}/{funTotalChecks(checklist)})
          <button
            onClick={toggleVisibility}
            className={`${isVisible ? StylesCard.hideBut : StylesCard.showBut}`}
            style={{
              width: "21px",
              height: "21px",
              position: "relative",
          ...
          <div>
            ...
            {checklist.map((taskList, index) => (
              <TaskList
                ...
                taskListId={myTaskId}
                checkListId={taskList._id}
                myChecklistDisplay={funTotalChecksMarked}
              />
            ))}
          </div>
        )}
        ...
        <br />
        <div
          className={StylesCard.cardFooter}
          style={{
            display: "flex",
        ...
        >
          {newDueDate ? (
            <div
              className={
                dueDatePassed && changeBoard !== "done"
                  ? StylesCard.butFooterDatePassed
                  : changeBoard === "done"
                  ? StylesCard.butFooterDateGreen
                  : priority === "HIGH PRIORITY"
                  ? StylesCard.butFooterDatePassed
                  : StylesCard.butFooterDate
              }
            >
              {newDueDate}
            </div>
      ...
      >
        <div>
          <div className={StylesCard.logoutText}>
            Are you sure you want to Delete?
          </div>
          <br />
          <div
            className={StylesCard.yesLogout}
            onClick={() => deleteTask(myTaskId)}
          >
            Yes, Delete
          </div>
          <br />
          <div className={StylesCard.cancel} onClick={closeModal}>
            Cancel
          </div>
        </div>
      </Modal>
